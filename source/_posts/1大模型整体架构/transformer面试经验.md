...
### 1. FFN 结构
假设输入维度为 \( d_{\text{model}} \)，FFN 的结构通常是：

1. 第一层的线性变换将维度从 \( d_{\text{model}} \) 映射到一个较大的维度 \( d_{\text{ffn}} \)。
2. 激活函数（通常是ReLU或GELU）在第一层和第二层之间。
3. 第二层的线性变换将维度从 \( d_{\text{ffn}} \) 映射回 \( d_{\text{model}} \)。
...
- 激活函数是对称的（如 tanh 或 sigmoid）。
  ...
  \]
  - \(n_{\text{in}} = d_{\text{model}}\)：输入维度
  - \(n_{\text{out}} = d_{\text{ffn}}\)：输出维度
...
- PyTorch 中的 `torch.nn.Linear` 默认使用 **Kaiming Uniform** 初始化：
  \[
  W \sim \mathcal{U}\left(-\sqrt{\frac{1}{n_{\text{in}}}}, \sqrt{\frac{1}{n_{\text{in}}}}\right)
  \]
...
- **线性层（`torch.nn.Linear`）**
  - 权重：使用均匀分布初始化，范围为 \([- \sqrt{\frac{1}{\text{fan\_in}}}, \sqrt{\frac{1}{\text{fan\_in}}}]\)，其中 `fan_in` 是输入的特征数量。
  - 偏置：初始化为 0。
